...
# Calculates and returns the CIDEr score for a single sentence
def calc_sent_cider(hyp, ref):
    ...
    res_ = [{'image_id':0, 'caption': res[0]}]
    res_test = [{'image_id':0, 'caption': gts[0]}]
    gts_ = {0: gts[0]}

    avg_scores, cider_scores = CiderD_scorer.compute_score(gts_, res_)
    test_scores, test_val = CiderD_scorer.compute_score(gts_, res_test)
